		...
		<div class="px-8 mt-6">
			<div v-for="(poeme, key) in poemes" class="border px-4 py-3 w-full flex justify-between mb-4 shadow-sm rounded" :key="key">
				...
				<div class="flex flex-row">
					...
					<button @click.prevent="openModal(poeme.id)" class="bg-red-600 text-white mx-1 px-2 py-1 rounded-md hover:bg-red-500"><i class="icon-trash text-white-500 bg-red-500"></i></button>
				</div>
					...
					<div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
						...
						>
							...
							<div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
								<button
									type="button"
									@click.prevent="handleDelete(poeme.id)"
...
export default {
	...
	setup() {
		...
		return { poemes, handleDelete, openModal, closeModal, state }
	},
}
</script>
